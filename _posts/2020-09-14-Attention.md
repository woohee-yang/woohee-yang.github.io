...
- 위 그림은 디코더의 세번째 LSTM 셀에서 출력을 예측할 때, 어텐션 메커니즘을 사요하는 모습을 보여준다. 
...
$$score(s_{t}, h_{i}) = s^{T}_{i} h_{i}$$

- $s_{t}$와 인코더의 모든 은닉 상태의 어텐션 스코어 모음값을 $e^{t}$라 정의하자.

$$e^{t} = [s^{T}_{t}h_{1}, ..., s^{T}_{t}h_{N}]$$

## 4.2 소프트맥스 함수를 토앻 어텐션 분포(Attention Distribution) 구하기
...
$$\alpha^{t} = \sum^{N}_{i=1} \alpha^{t}_{i}h_{i}$$
...
$$\tilde{s_{t}} = tanh(W_{c}|\alpha_{t};s_{t}| + b_{c})$$
...
$$\hat{y_{t}} = Softmax(W_{s}\tilde{s_{t}} + b_{y})$$
